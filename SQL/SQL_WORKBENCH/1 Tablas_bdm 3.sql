...
(
    ...
    ultima_fecha_ingreso DATE DEFAULT NULL,               -- Última fecha de ingreso al curso
    ultima_hora_inreso TIME DEFAULT NULL,
    fecha_terminacion_inscripcion DATE DEFAULT NULL,      -- Fecha de terminación de la inscripción
    hora_terminacion_inscripcion TIME DEFAULT NULL,
    inscripcion_finalizada BIT DEFAULT 0,		              -- Indica si la inscripción está finalizada ( 0=NO TERMINADO, 1=TERMINADO,)
    metodo_pago_inscripcion INT NOT NULL,                     -- Método de pago de la inscripción ( 0 = TARJETA DEBITO,1 = TARJETA CREDITO, 2 = PAYPAL, 3 = Gratis)
...
CREATE TABLE tabla_niveles_inscripcion 
(
    id_nivel_inscripcion INT NOT NULL AUTO_INCREMENT PRIMARY KEY, -- ID único para esta relación
...
(
    ...
    id_usuario1_mensaje INT NOT NULL,                      -- ID ALUMNO MENSAJE (llave foránea a la tabla "usuarios")
    id_usuario2_mensaje INT NOT NULL,                      -- ID INSTRUCTOR MENSAJE (llave foránea a la tabla "usuarios")
    fecha_creacion_mensaje DATETIME,					     -- Fecha de creación del mensaje
    FOREIGN KEY (id_usuario1_mensaje) REFERENCES tabla_usuario(id_usuario),  -- FK a la tabla "usuarios" (alumno)
    FOREIGN KEY (id_usuario2_mensaje) REFERENCES tabla_usuario(id_usuario)	-- FK a la tabla "usuarios" (instructor)
);